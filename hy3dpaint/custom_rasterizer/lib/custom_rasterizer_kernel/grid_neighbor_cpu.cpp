    ...
    std::vector<torch::Tensor> view_layer_normals, int num_level, int resolution)
{
    ...
    texture_positions[0] = torch::zeros({static_cast<long long>(seq2pos.size()) / 3, 3}, float_options);
    texture_positions[1] = torch::zeros({static_cast<long long>(seq2pos.size()) / 3}, float_options);
...
//        grid_neighbors[i] = torch::zeros({grids[i].seq2grid.size(), 9}, int64_options);
        grid_neighbors[i] = torch::zeros({static_cast<long long>(grids[i].seq2grid.size()), 9}, int64_options);
...
//        grid_oddcorners[i] = torch::zeros({grids[i].seq2oddcorner.size()}, int64_options);
        grid_evencorners[i] = torch::zeros({static_cast<long long>(grids[i].seq2evencorner.size())}, int64_options);
        grid_oddcorners[i] = torch::zeros({static_cast<long long>(grids[i].seq2oddcorner.size())}, int64_options);
...
//            grid_downsamples[i] = torch::zeros({grids[i].downsample_seq.size()}, int64_options);
            grid_downsamples[i] = torch::zeros({static_cast<long long>(grids[i].downsample_seq.size())}, int64_options);
    ...
    int num_level, int resolution)
{
    ...
    texture_positions[0] = torch::zeros({static_cast<long long>(seq2pos.size()) / 3, 3}, float_options);
    texture_positions[1] = torch::zeros({static_cast<long long>(seq2pos.size()) / 3}, float_options);
    texture_feats[0] = torch::zeros({static_cast<long long>(seq2feat.size()) / feat_channel, feat_channel}, float_options);
//    texture_positions[0] = torch::zeros({seq2pos.size() / 3, 3}, float_options);
//    texture_positions[1] = torch::zeros({seq2pos.size() / 3}, float_options);
